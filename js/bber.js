if (document.querySelector('#bber-loading')) {
  ...
  if (document.querySelector('#bber')) {
    app.auth({
      ...
      var bbClass = '#bber'
      $(bbClass).after('<div class="load"><button class="load-btn button-load">Loading 1……</button></div>')
      ...
      collection.count(function(err,res){
        ...
        $(bbClass).append('<p class="count">Total: <span class="count-data">'+count+'</span> talks</p>')
        getList()
      })
      function getList(){
        if((page-1)*per >= count){
        ...
        collection.limit(per).skip((page-1)*per).orderBy('date','desc').get(function(err, res) {
          ...
          $('button.button-load').text('Load more')

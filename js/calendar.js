...
(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
      define([], function () {
        return (root.CalendarUI = factory(root));
      });
    } else {
      root.CalendarUI = factory(root);
    }
  }(typeof window !== 'undefined' ? window : this, function (root, undefined) {
    "use strict";
  
    var CalendarUI = (function (id, opts) {
  
      opts = opts || {};
  
      if(!id) {
        throw Error('CalendarUI: Container id is required as first argument.');
      }
  
      if(!document.getElementById(id)) {
        throw Error('CalendarUI: Element with id #' + id + ' does not exist.');
      }
  
      if(!root.Vue && !opts.vue) {
        throw Error('CalendarUI: Vue is required but not found.');
      }
      ...
      _interface = {}, // methods returned by a CalendarUI() instance.
      _actionResolve,
      _markDownRegex = {
        icon: /!\(([^\)]+)\)/igm, // !(icon)
      ...
      function _linkReplacer(match, $1, $2, $3) {
        ...
        return "<a class='calendar-message-content-link' target='" + _target + "' href='" + $2 +"'>" + $1 + "</a>";
      }
  
      function _parseMarkDown(text) {
        return text
                   .replace(_markDownRegex.image, "<img class='calendar-message-content-image' src='$2' alt='$1' />")
                   .replace(_markDownRegex.icon, "<i class='calendar-icon calendar-message-content-icon fa fa-$1'></i>")
      ...
      var _calendarComponent = {
        template: '<div class=\"calendar calendar-container\" v-calendar-container><div class=\"calendar-messages-container\"><div v-for=\"msg in messages\" class=\"calendar-message\" :class=\"msg.cssClass\" v-calendar-scroll><transition name=\"slide-fade\"><div v-if=\"msg.visible\"><div v-if=\"msg.photo && !msg.loading\" :class=\"[\'profil\', \'profile\', {human: msg.human, \'agent\': !msg.human}]\"> <img :src=\"msg.photo\" :class=\"[{human: msg.human, \'agent\': !msg.human}]\"></div><div :class=\"[{human: msg.human, \'calendar-message-content\': true}, msg.type]\"><span v-if=\"msg.type == \'text\'\" v-text=\"msg.content\" v-calendar-markdown></span><span v-if=\"msg.type == \'html\'\" v-html=\"msg.content\"></span> <iframe v-if=\"msg.type == \'embed\'\" :src=\"msg.content\" frameborder=\"0\" allowfullscreen></iframe></div></div></transition><div v-if=\"msg.photo && msg.loading && !msg.human\" :class=\"[\'profil\', \'profile\', {human: msg.human, \'agent\': !msg.human}]\"> <img :src=\"msg.photo\" :class=\"[{human: msg.human, \'agent\': !msg.human}]\"></div><div v-if=\"msg.loading\" class=\"calendar-message-content loading\"><i class=\"dot\"></i><i class=\"dot\"></i><i class=\"dot\"></i></div></div></div><div class=\"calendar-actions-container\"><transition name=\"slide-fade\"><div v-if=\"action.show\" v-calendar-scroll><form v-if=\"action.type == \'text\'\" class=\"calendar-actions-text\" @submit.prevent=\"handle_action_text()\" :class=\"action.cssClass\"><i v-if=\"action.text.icon\" class=\"calendar-icon calendar-action-text-icon fa\" :class=\"\'fa-\' + action.text.icon\"></i> <input type=\"text\" ref=\"input\" :type=\"action.text.sub_type\" v-model=\"action.text.value\" class=\"calendar-actions-text-input\" :placeholder=\"action.text.placeholder\" :size=\"action.text.size\" :value=\" action.text.value\" :class=\"action.text.cssClass\" required v-focus/> <button type=\"submit\" :class=\"{\'calendar-actions-buttons-button\': !!action.text.button, \'calendar-actions-text-submit\': !action.text.button}\"><i v-if=\"action.text.button && action.text.button.icon\" class=\"calendar-icon calendar-action-button-icon fa\" :class=\"\'fa-\' + action.text.button.icon\"></i> <span>{{(action.text.button && action.text.button.label) || \'Go\'}}</span></button></form><form v-if=\"action.type == \'select\'\" class=\"calendar-actions-select\" @submit.prevent=\"handle_action_select()\" :class=\"action.cssClass\"><i v-if=\"action.select.icon\" class=\"calendar-icon calendar-action-select-icon fa\" :class=\"\'fa-\' + action.select.icon\"></i><v-select v-if=\"action.select.searchselect && !action.select.multipleselect\" v-model=\"action.select.value\" :value=\"action.select.value\" :placeholder=\"action.select.placeholder\" class=\"calendar-actions-text-searchselect\" :label=\"action.select.label\" :options=\"action.select.options\"></v-select><v-select v-else-if=\"action.select.searchselect && action.select.multipleselect\" multiple v-model=\"action.select.value\" :value=\"action.select.value\" :placeholder=\"action.select.placeholder\" class=\"calendar-actions-text-searchselect\" :label=\"action.select.label\" :options=\"action.select.options\"></v-select> <select v-else v-model=\"action.select.value\" class=\"calendar-actions-text-select\" :placeholder=\"action.select.placeholder\" :size=\"action.select.size\" :class=\"action.select.cssClass\" required v-focus><option v-for=\"option in action.select.options\" :class=\"action.select.optionClass\" v-bind:value=\"option.value\" :disabled=\"(option.value == \'\')?true:false\" :selected=\"(action.select.value == option.value)?\'selected\':\'\'\"> {{ option.text }}</option></select> <button type=\"submit\" :class=\"{\'calendar-actions-buttons-button\': !!action.select.button, \'calendar-actions-select-submit\': !action.select.button}\"><i v-if=\"action.select.button && action.select.button.icon\" class=\"calendar-icon calendar-action-button-icon fa\" :class=\"\'fa-\' + action.select.button.icon\"></i> <span>{{(action.select.button && action.select.button.label) || \'Ok\'}}</span></button></form><div v-if=\"action.type == \'button\'\" class=\"calendar-actions-buttons\" :class=\"action.cssClass\"> <button type=\"button\" :class=\"button.cssClass\" class=\"calendar-actions-buttons-button\" v-calendar-scroll v-for=\"button in action.button.buttons\" @click=\"handle_action_button(button)\"><i v-if=\"button.icon\" class=\"calendar-icon calendar-action-button-icon fa\" :class=\"\'fa-\' + button.icon\"></i> {{button.text}}</button></div><form v-if=\"action.type == \'buttontext\'\" class=\"calendar-actions-text\" @submit.prevent=\"handle_action_text()\" :class=\"action.cssClass\"><i v-if=\"action.text.icon\" class=\"calendar-icon calendar-action-text-icon fa\" :class=\"\'fa-\' + action.text.icon\"></i> <input type=\"text\" ref=\"input\" :type=\"action.text.sub_type\" v-model=\"action.text.value\" class=\"calendar-actions-text-input\" :placeholder=\"action.text.placeholder\" :size=\"action.text.size\" :value=\"action.text.value\" :class=\"action.text.cssClass\" required v-focus/> <button type=\"submit\" :class=\"{\'calendar-actions-buttons-button\': !!action.text.button, \'calendar-actions-text-submit\': !action.text.button}\"><i v-if=\"action.text.button && action.text.button.icon\" class=\"calendar-icon calendar-action-button-icon fa\" :class=\"\'fa-\' + action.text.button.icon\"></i> <span>{{(action.text.button && action.text.button.label) || \'Go\'}}</span></button><div class=\"calendar-actions-buttons\" :class=\"action.cssClass\"> <button type=\"button\" :class=\"button.cssClass\" class=\"calendar-actions-buttons-button\" v-for=\"button in action.button.buttons\" @click=\"handle_action_button(button)\" autofocus><i v-if=\"button.icon\" class=\"calendar-icon calendar-action-button-icon fa\" :class=\"\'fa-\' + button.icon\"></i> {{button.text}}</button></div></form></div></transition></div></div>', // replaced by HTML template during build. see Gulpfile.js
        data: function () {
          return {
            action: {
              text: {
      ...
      root.Vue.directive('calendar-markdown', function (el, binding) {
        if(binding.value == 'false') return; // v-calendar-markdown="false"
        el.innerHTML = _parseMarkDown(el.textContent);
      });
  
      root.Vue.directive('calendar-scroll', {
      ...
      root.Vue.directive('calendar-container', {
        inserted: function (el) {
          _container = el;
        }
      });
      ...
      _botApp = new root.Vue({
        components: {
          'calendar-ui': _calendarComponent
        }
      }).$mount('#' + id);
      ...
      function _addMessage(_msg) {
        ...
        if(!_msg.loading && !_msg.content) {
          throw Error('CalendarUI: "content" is required in a non-loading message object.');
        }
      ...
      function _checkAction(_opts) {
        if(!_opts.action && !_opts.actionButton  && !_opts.actionText) {
          throw Error('CalendarUI: "action" property is required.');
        }
      }
